...
async function processEchoBotV2(blob) {
    try {
        ...
        let errorMessage = 'Network or service error occurred';
        let fallbackMessage = "I'm having trouble processing your audio right now. Please try again in a moment.";
        
        // Handle different error types
        if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
            errorMessage = 'Unable to connect to the service. Please check your internet connection.';
        } else if (error.message.includes('timeout')) {
            errorMessage = 'Request timed out. The service may be busy.';
            fallbackMessage = "The service is taking longer than expected. Please try again.";
        } else if (error.message.includes('401')) {
            errorMessage = 'Authentication error. API key may be invalid.';
        } else if (error.message.includes('429')) {
            errorMessage = 'Rate limit exceeded. Please wait a moment.';
            fallbackMessage = "Too many requests right now. Please wait a moment and try again.";
        } else if (error.message.includes('500')) {
            errorMessage = 'Server error. The service is experiencing issues.';
            fallbackMessage = "I'm experiencing some technical difficulties. Please try again later.";
        }
        
        updateRecordingStatus('error', `Echo Bot error: ${errorMessage}`);
        showNotification(`Echo Bot failed: ${errorMessage}`, 'error');
        
        // Try to provide fallback audio response
        if (window.speechSynthesis && 'speechSynthesis' in window) {
            ...
            setTimeout(() => {
                ...
                showNotification('Using browser speech synthesis as fallback', 'info');
            }, 1000);
        }
    }
}
