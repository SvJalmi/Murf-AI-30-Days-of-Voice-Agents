document.addEventListener("DOMContentLoaded", () => {
    ...
    weatherBtn.addEventListener("click", async () => {
        ...
        displayMessage(`🌤️ Getting weather for: ${location}`);
        weatherInput.value = ""; // Clear input
        ...
        try {
            ...
            const data = await response.json();
            if (response.ok) {
                displayMessage(`Weather Info: ${data.weather}`, "success");
                playAudio(data.audio_url);
            } else {
                displayMessage(`Error: ${data.error}`, "error");
            }
    ...
    // Allow weather query with Enter key
    ...
    displayMessage("🎭🔍🌤️ Welcome to the AI Voice Agent with Dual Special Skills!", "info");
    displayMessage("Choose a persona and try the web search or weather information features!", "info");
});
